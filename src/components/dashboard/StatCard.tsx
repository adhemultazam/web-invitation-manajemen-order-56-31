    ...
    <Card className={cn(`overflow-hidden border rounded-xl shadow-sm stat-card ${getBackgroundClass()}`, className)}>
      <CardContent className="p-0.5 sm:p-1 md:p-1.5">
        <div className="flex justify-between items-center mb-0.5">
          <h3 className="text-[8px] font-medium text-gray-600 dark:text-gray-400 font-poppins">{title}</h3>
          <div className={`rounded-full w-2.5 h-2.5 md:w-3 md:h-3 flex items-center justify-center shadow-md ${getIconColorClass()}`}>
            <span className="scale-[60%]">{icon}</span>
          </div>
        </div>
        <div className="flex items-end justify-between">
          <div className="animate-count-up" style={{ animationDelay: '0.2s' }}>
            <div className={cn(
              "text-[9px] sm:text-[10px] md:text-xs font-bold overflow-hidden text-ellipsis whitespace-nowrap",
              getTextColorClass(),
              "font-poppins"
            )}>
              {formatValue()}
            ...
            {description && (
              <p className="text-[6px] mt-0.5 text-gray-500 dark:text-gray-400 font-inter">
                {description}
              </p>
            )}
          </div>
